...
function mailbox(channel: EmailChannel) {
  ...
  if (channel === "orders") return process.env.SMTP_ORDERS_USER || "orders@healthfieldpharmacy.co.ke";
  if (channel === "security") return process.env.SMTP_SECURITY_USER || "security@healthfieldpharmacy.co.ke";
  return fallbackUser;
}

function configuration(channel: EmailChannel = "general") {
  const host = process.env.SMTP_HOST;
  const user = mailbox(channel);
  const pass = process.env.SMTP_PASSWORD;
  if (!host || !user || !pass) return null;
  const port = Number(process.env.SMTP_PORT || 465);
  const label = channel === "security" ? "Healthfield Security" : channel === "orders" ? "Healthfield Orders" : process.env.SMTP_FROM_NAME || "Healthfield Pharmacy";
  return {
    from: `"${label.replaceAll('"', "")}" <${user}>`,
    transport: nodemailer.createTransport({ host, port, secure: process.env.SMTP_SECURE ? process.env.SMTP_SECURE === "true" : port === 465, auth: { user, pass }, pool: true, maxConnections: 3, maxMessages: 100 }),
  };
}
...
export async function sendEmail(input: EmailInput) {
  ...
  try {
    ...
    config.transport.close();
    console.error(`[email:${channel}] SMTP send failed:`, input.subject, error);
