...
function ActivationContent() {
  ...
  const [message, setMessage] = useState(
    deliveryFailed
      ? "We could not send the first activation email. Use the button below to send a fresh link."
      : "",
  );
  const [deliveryProblem, setDeliveryProblem] = useState(deliveryFailed);
  const [resending, setResending] = useState(false);
  const [cooldown, setCooldown] = useState(
    deliveryFailed ? 0 : registered ? 60 : 0,
  );
    ...
    void fetch("/api/auth/verify-email", {
      ...
      .then(({ ok, data }) => {
        ...
        setMessage(
          data.message ||
            data.error ||
            "Email activation could not be completed.",
        );
      })
      .catch(() => {
        if (cancelled) return;
        setScreen("error");
        setMessage(
          "We could not reach the activation service. Please try again.",
        );
      });
    return () => {
      cancelled = true;
  ...
  async function resend(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await response
        .json()
        .catch(() => ({}))) as ActivationResponse;
      if (!response.ok) {
        setDeliveryProblem(data.code === "ACTIVATION_DELIVERY_FAILED");
        if (
          data.code === "ACTIVATION_COOLDOWN" &&
      ...
      setMessage(
        data.message || `A fresh activation link has been sent to ${email}.`,
      );
      setScreen("awaiting");
    } catch {
  ...
  const description = activated ? (
    ...
    <>
      We sent an activation link to <strong>{email}</strong>. Kindly check your
      inbox and select the link to activate your account.
    </>
  ) : (
  ...
  return (
    <main className="verify-page">
      <section className="registration-notice">
        ...
        <span
          className={`registration-mail-icon ${screen === "error" || deliveryProblem ? "is-error" : ""}`}
        >
          {screen === "error" || deliveryProblem ? (
            <AlertCircle />
        ...
        {!activated && !verifying ? (
          <small>
            The secure activation link expires after 24 hours. You can request a
            new one if it does not arrive.
          </small>
        ) : null}
        {message && !activated ? (
          <div
        ...
        {email && !activated && !verifying ? (
          <form className="activation-resend" onSubmit={resend}>
        ...
        <div className="activation-actions">
          <Link className="verify-login" href={loginHref}>
            <LogIn />
            Continue to login
          </Link>
          <Link className="verify-guest" href="/#products">
            <ShoppingBag />
            Shop as a guest
          </Link>
        </div>
...
export default function VerifyEmailPage() {
  return (
    <Suspense
      fallback={
        <main className="verify-page">
          <section className="registration-notice">Loading…</section>
        </main>
      }
    >
      <ActivationContent />
