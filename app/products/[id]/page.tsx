...
export default async function ProductPage({
  ...
  const cartCount = Object.values(cart).reduce<number>(
  ...
  const discountPercent =
    product.discountPrice && regularPrice > Number(product.discountPrice)
      ? Math.round((1 - Number(product.discountPrice) / regularPrice) * 100)
      : 0;
  ...
  const productSchema = {
    ...
    offers: {
      ...
      seller: { "@type": "Organization", name: "Healthfield Pharmacy" },
    },
    ...(rating && reviewCount
      ? {
          aggregateRating: {
    ...
    <>
      <main className="product-detail">
        ...
        <header className="product-page-header">
          ...
          <Link className="product-header-logo" href="/">
            <Image
        ...
        <section className="product-primary">
          <div className="product-detail-image">
            {product.imageUrl ? (
              ...
              <div>
                <Package />
                <span>Product image has not been added</span>
              </div>
            )}
          </div>
          <div className="product-detail-copy">
            <span>{product.brand || "Healthfield Pharmacy"}</span>
            <h1>{product.name}</h1>
            {rating && (
            ...
            <div className="product-price-row">
              ...
              {discountPercent > 0 && (
                <>
                  <del>KES {regularPrice.toLocaleString()}</del>
                  <span className="detail-discount">
                    Save {discountPercent}%
                  </span>
                </>
              )}
              <span className="availability-pill">Available to order</span>
            </div>
            {product.prescriptionRequired && (
              <div className="prescription-note">
                <ShieldCheck />
                <span className="prescription-note-copy">
                  <span className="prescription-note-full">A valid prescription is required before this order can be processed.</span>
        ...
        <section className="product-reviews">
          <header>
            <div>
              ...
              <p>
                {reviewCount
                  ? `${rating?.toFixed(1)} from ${reviewCount} verified customer reviews`
            ...
            <div className="review-list">
              {reviews.map((review) => (
                <article key={review.id}>
                  ...
                  <p>{review.comment}</p>
                  <small>
                    {review.firstName} · verified purchase ·{" "}
        ...
        <ProductRail
          ...
          subtitle="Products customers often purchase in the same order."
          items={bought}
        />
        <ProductRail
          title="People also like"
          subtitle="Popular choices for similar health needs."
          items={similar}
        />
        <ProductRail
          title="More from this category"
          subtitle="Keep exploring products selected for you."
          items={related}
        />
      </main>
      {home?.contact && (
        <PublicFooter contact={home.contact} signedIn={!!session} />
      )}
    </>
  );
}
