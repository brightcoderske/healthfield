...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  try{[products,posts,stores]=await Promise.all([backendJson<{products:Array<{id:number;updatedAt:string}>}>("/v1/views/sitemap").then(value=>value.products),backendJson<{posts:Array<{slug:string;updatedAt:string}>}>("/v1/views/blogs").then(value=>value.posts),backendJson<{stores:Array<{name:string;code:string;updatedAt:string}>}>("/v1/views/locations").then(value=>value.stores)]);}catch{}
