...
export function CartView({
  ...
  const lines = useMemo(
    () =>
      Object.entries(cart)
        .map(([id, quantity]) => ({
          product: catalog.find((product) => product.id === Number(id)),
          quantity,
        }))
  ...
  const prescriptionLines = lines.filter(
    (line) => line.product.prescriptionRequired,
  );
      ...
      <div className="cart-layout">
        <section>
          {initialOffers.map((offer) => (
            <article
              key={`offer-${offer.id}`}
              className={`cart-bundle${offer.items.some((item) => item.prescriptionRequired) ? " cart-prescription-line" : ""}`}
            >
              <span className="cart-image cart-bundle-badge">
                <Tag />
              </span>
              <div>
              ...
              <div className="cart-quantity cart-bundle-quantity">
                <span>1</span>
                <small>bundle</small>
              </div>
              <form
            ...
            >
              ...
              <div>
                ...
                {line.product.packSize && (
                  <small>{line.product.packSize}</small>
                )}
                {line.product.prescriptionRequired ? (
                  <>
                    <b>Excluded from cart total</b>
                  ...
                  <b>
                    KES{" "}
                    {Number(
                      line.product.discountPrice ?? line.product.price,
                    ).toLocaleString()}
                  </b>
                )}
              </div>
              {line.product.prescriptionRequired ? (
                <div className="cart-prescription-status">
        ...
        <aside>
          ...
          initialOffers.length !== payableOffers.length ? (
            <p className="cart-prescription-summary">
              Prescription medicines are excluded until a pharmacist reviews the
