...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = JSON.parse(
        sessionStorage.getItem(twoFactorStorageKey) || "null",
      ) as StoredChallenge | null;
      if (
        stored &&
        /^.{60,100}$/.test(stored.challengeToken) &&
        stored.maskedEmail &&
        Number.isFinite(stored.expiresAtMs) &&
        Number.isFinite(stored.resendAvailableAtMs) &&
        Number.isFinite(stored.challengeEndsAtMs) &&
        stored.challengeEndsAtMs > Date.now()
      ) {
  ...
  const secondsRemaining = Math.max(
    0,
    Math.ceil((expiresAtMs - clockMs) / 1000),
  );
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        setActivationEmail(
          data.code === "EMAIL_NOT_VERIFIED" ? submittedEmail : "",
        );
        setError(
          data.error ??
            "Login could not be completed. Check your details and try again.",
        );
        setLoading(false);
        return;
      }
      ...
      if (data.requiresTwoFactor) {
        if (
          ...
          typeof data.resendAvailableAtMs !== "number" ||
          typeof data.challengeEndsAtMs !== "number"
        )
          throw new Error("Invalid two-factor challenge response.");
        const stored = {
          challengeToken: data.challengeToken,
          maskedEmail: data.maskedEmail,
          expiresAtMs: data.expiresAtMs,
          resendAvailableAtMs: data.resendAvailableAtMs,
          challengeEndsAtMs: data.challengeEndsAtMs,
      ...
      if (
        ...
        throw new Error("Invalid login redirect.");
      window.location.assign(
        data.redirectTo === "/change-password"
          ? data.redirectTo
          : safeRequested || data.redirectTo,
      );
    } catch {
      setError(
        "Healthfield could not connect to the login service. Please try again.",
      );
      setLoading(false);
    } finally {
  ...
  async function verifyCode(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/auth/two-factor", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ challengeToken, code: securityCode }),
      });
      const data = (await response.json().catch(() => ({}))) as AuthResponse;
      if (!response.ok) {
        const message = String(
          data.error ?? "The security code could not be verified.",
        );
        if (data.code === "TWO_FACTOR_ENDED") {
          clearChallenge();
          setError(
            "That verification request has ended. Sign in again to receive a fresh code.",
          );
          setLoading(false);
          return;
        }
      ...
      if (
        ...
        throw new Error("Invalid verification redirect.");
      sessionStorage.removeItem(twoFactorStorageKey);
      window.location.assign(
        data.redirectTo === "/change-password"
          ? data.redirectTo
      ...
      setError(
        "Healthfield could not verify the security code. Please try again.",
      );
      setLoading(false);
    } finally {
  ...
  async function resendCode() {
    ...
    try {
      ...
      if (!response.ok) {
        if (
          data.code === "TWO_FACTOR_ENDED" ||
          data.code === "TWO_FACTOR_DELIVERY_FAILED"
        )
          clearChallenge();
        if (
          data.code === "TWO_FACTOR_COOLDOWN" &&
          typeof data.retryAfterSeconds === "number"
        )
          setResendAvailableAtMs(Date.now() + data.retryAfterSeconds * 1000);
        ...
        setResendAvailableAtMs(data.resendAvailableAtMs);
        setClockMs(Date.now());
        setNotice("A fresh code was sent. Only this new code will work.");
      }
    } catch {
  ...
  return (
    <main className="auth-page">
        ...
        {challengeToken ? (
          <>
            <p className="two-factor-intro">
              ...
              The newest email contains the only valid code.
            </p>
            <div
              className={`two-factor-timer${codeExpired ? " is-expired" : ""}`}
              role="status"
            >
              <span>{codeExpired ? "Code expired" : "Code expires in"}</span>
              <strong>{codeExpired ? "Send a new code" : expiryLabel}</strong>
            </div>
            <form onSubmit={verifyCode} className="two-factor-form">
              <label>
                <span>Security code</span>
                <div className="two-factor-code-field">
                  <KeyRound />
                  <input
              ...
              <small id="two-factor-help" className="two-factor-help">
                You can type or paste the code from your email.
              </small>
              {error && (
              ...
              <button
                ...
                disabled={loading || securityCode.length !== 6 || codeExpired}
              >
                {loading
                  ? "Verifying…"
                  : codeExpired
            ...
            <div className="two-factor-options">
              ...
              >
                {resendWaitSeconds > 0
                  ? `Send again in ${resendWaitSeconds}s`
                  : "Send a new code"}
              </button>
              <button
                type="button"
          ...
          <>
                  ...
                  {error ||
                    (urlError === "incorrect"
                      ...
                      : urlError === "session_expired"
                        ? "Your previous session is no longer valid. Sign in again securely."
                        : "Enter a valid email and password.")}
                </div>
              )}
              {activationEmail ? (
                <Link
                  className="activation-help-link"
                  href={`/verify-email?email=${encodeURIComponent(activationEmail)}${safeRequested ? `&next=${encodeURIComponent(safeRequested)}` : ""}`}
                >
            ...
            <Link
              ...
              href={
                safeRequested
                  ? `/register?next=${encodeURIComponent(safeRequested)}`
                  : "/register"
              }
            >
      ...
      <aside>
        <div>
          <span>Healthfield Pharmacy</span>
          <h2>Order medicines and health essentials online.</h2>
          <p>Log in to view your orders, saved products and prescriptions.</p>
        </div>
      </aside>
    </main>
  );
