...
import { ArrowLeft, Check, CheckCircle2, Clipboard, CreditCard, LoaderCircle, LocateFixed, MapPin, ReceiptText, RefreshCw, ShoppingBag, ShoppingCart, Smartphone } from "lucide-react";
...
export type CheckoutOffer = { id: number; title: string; total: number; items: Array<{ productId: number; name: string; quantity: number }> };
...
export function CheckoutForm({ initialCart, initialCatalog, initialOffers = [], customer, payment }: { initialCart: Record<number, number>; initialCatalog: Product[]; initialOffers?: CheckoutOffer[]; customer: Customer; payment: PaymentOptions }) {
  ...
  const bundleTotal = initialOffers.reduce((sum, offer) => sum + Number(offer.total), 0);
  const subtotal = lines.reduce((sum, line) => sum + Number(line.product!.discountPrice ?? line.product!.price) * line.quantity, 0) + bundleTotal;
  ...
  async function retryPayment() {
    setSubmitting(true); setError("");
    const response = await fetch("/api/payments/retry", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ checkoutToken: checkoutToken.current, billingPhone: retryPhone }) }).catch(() => null);
    const data = await response?.json().catch(() => ({}));
    if (!response?.ok) {
      setError(data?.error || "The payment prompt could not be sent.");
    } else if (data.paymentStatus === "PAID" || data.order?.paymentStatus === "PAID") {
      await clearCheckoutCart();
      setResult((current) => current ? { ...current, state: "PAID", message: "Payment confirmed. Your order is now being processed." } : current);
    } else {
      checkoutToken.current = data.checkoutToken || checkoutToken.current;
      pollCount.current = 0;
      setResult((current) => current ? { ...current, state: "WAITING", message: data.message || "Approve the new payment prompt on your phone." } : current);
    }
    setSubmitting(false);
  }
