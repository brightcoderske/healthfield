...
export function SettingsForm({ initial, paymentRuntime }: { initial: SettingsValue; paymentRuntime: { mpesaConfigured: boolean } }) {
  ...
  async function registerC2b(){setRegisteringC2b(true);setMessage("");const response=await fetch("/api/payments/c2b/register",{method:"POST"}).catch(()=>null);const data=await response?.json().catch(()=>({}));setMessage(response?.ok?data.message||"Safaricom C2B callback URLs registered.":data?.error||"C2B callback URLs could not be registered.");setRegisteringC2b(false)}
  return (
