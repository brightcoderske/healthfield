...
export function StaffManager({ initialStaff, stores, canManageAdmins }: { initialStaff:Staff[]; stores:Store[]; canManageAdmins:boolean }) {
  ...
  const [permissionsEditing,setPermissionsEditing]=useState<Staff|null>(null);
  ...
  function openEditor(staff:Staff|"new"){
    ...
    setEditing(staff);setConfirmDelete(false);setError("");setNotice("");
  }
  function openPermissions(staff:Staff){setPermissionsEditing(staff);setPermissionDraft(staff.permissions);setError("");setNotice("")}
  function togglePermission(permission:StaffPermission){setPermissionDraft((current)=>current.includes(permission)?current.filter((item)=>item!==permission):[...current,permission])}

  async function savePermissions(){
    if(!permissionsEditing||permissionSaving)return;
    setPermissionSaving(true);setError("");
    try{
      const response=await fetch(`/api/staff/${permissionsEditing.id}/permissions`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({permissions:permissionDraft})});
      const data=await response.json().catch(()=>({}));
      if(!response.ok){setError(data.error||"Staff permissions could not be saved.");return}
      setRows((items)=>items.map((row)=>row.id===permissionsEditing.id?{...row,permissions:data.permissions}:row));
      setNotice(`${permissionsEditing.firstName}'s permissions were saved. Their current sessions were signed out.`);
      setPermissionsEditing(null);
    }catch{setError("Staff permissions could not be saved. Check the connection and try again.")}
  ...
  async function save(event:FormEvent<HTMLFormElement>){
    ...
    const isNew=editing==="new",id=isNew?"new":editing.id;setSaving(id);setError("");setNotice("");
    try{
      const response=await fetch(isNew?"/api/staff":`/api/staff/${id}`,{method:isNew?"POST":"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}),data=await response.json().catch(()=>({}));
      if(!response.ok){setError(data.error||"Staff account could not be saved.");return}
      if(isNew)setRows((items)=>[{...payload,id:data.id,phone:payload.phone||null,role:payload.role as Staff["role"],twoFactorEnabled:false,permissions:payload.role==="STAFF"?[...DEFAULT_STAFF_PERMISSIONS]:[]} as Staff,...items]);
      else setRows((items)=>items.map((row)=>row.id===id?{...row,...payload,phone:payload.phone||null,role:payload.role as Staff["role"],email:row.email,permissions:row.role!==payload.role&&payload.role==="STAFF"?[...DEFAULT_STAFF_PERMISSIONS]:payload.role==="ADMIN"?[]:row.permissions}:row));
      setEditing(null);
    }catch{setError("Staff account could not be saved. Check the connection and try again.")}
    finally{setSaving(null)}
  }

  async function remove(){
    if(!current)return;setSaving(current.id);setError("");
    try{const response=await fetch(`/api/staff/${current.id}`,{method:"DELETE"}),data=await response.json().catch(()=>({}));if(!response.ok){setError(data.error||"Staff account could not be deleted.");return}setRows((items)=>items.filter((row)=>row.id!==current.id));setEditing(null);setConfirmDelete(false)}
    catch{setError("Staff account could not be deleted. Check the connection and try again.")}
    finally{setSaving(null)}
  }
  ...
  return <main className="compact-admin-page">
    ...
    {notice?<div className="staff-permission-notice" role="status">{notice}</div>:null}
    ...
    <div className="compact-table"><div className="compact-table-head manager-row staff-manager-row"><span>Staff member</span><span>Role</span><span>Store</span><span>Phone</span><span>Status</span><span>Actions</span></div>{shown.map((row)=><div className={`compact-table-row manager-row staff-manager-row ${saving===row.id?"row-saving":""}`} key={row.id}>
      ...
      <span className="staff-row-actions">{row.role==="STAFF"?<button type="button" onClick={()=>openPermissions(row)}><KeyRound/> Roles <small>{row.permissions.length}</small></button>:<small>{row.role==="SUPER_ADMIN"?"Owner access":"Full admin access"}</small>}</span>
    </div>)}</div>

    {editing?<div className="product-modal" onClick={()=>setEditing(null)}><form onSubmit={save} onClick={(event)=>event.stopPropagation()}>
      <header><h2>{editing==="new"?"Add staff":"Edit staff"}</h2><button type="button" onClick={()=>setEditing(null)} aria-label="Close editor"><X/></button></header>
