...
export function UnmatchedPaymentsManager({ initialPayments, exceptions }: { initialPayments:UnmatchedPayment[]; exceptions:PaymentException[] }) {
  ...
  const shown=useMemo(()=>{const term=query.trim().toLowerCase();return term?payments.filter((payment)=>`${payment.receiptNumber} ${payment.phone||""} ${payment.accountReference||""} ${payment.amount} ${payment.candidates.map((candidate)=>`${candidate.orderNumber} ${candidate.customerName}`).join(" ")}`.toLowerCase().includes(term)):payments},[payments,query]);
