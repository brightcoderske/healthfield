...
export function OrderList({orders,statuses=allStatuses,filterKey="healthfield-order-status-filters",pageSize,emptyHint}:{orders:Order[];statuses?:readonly string[];filterKey?:string;pageSize?:number;emptyHint?:string}){
  const [q,setQ]=useState(""),[rows,setRows]=useState(orders),[open,setOpen]=useState<number|null>(null),[message,setMessage]=useState(""),[selected,setSelected]=useState<string[]>([]),[page,setPage]=useState(0);
  useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(filterKey)||"[]");if(Array.isArray(saved))setSelected(saved.filter((item:unknown):item is string=>statuses.includes(item as string)))}catch{}},[filterKey,statuses]);
  const counts=useMemo(()=>Object.fromEntries(statuses.map((status)=>[status,rows.filter((order)=>order.status===status).length])),[rows,statuses]);
  ...
  // Reset to the first page whenever the result set changes underneath the reader.
  useEffect(()=>{setPage(0)},[q,selected,rows]);
