...
import { allowedOrderStatuses, isStockFinalizedOrderStatus, orderDetailsAreEditable, type OrderFulfilmentMethod, type OrderStatus } from "@/lib/order-status-transitions";
...
export function OrderStatusManager({ order, items, stores = [], fulfilments = [], stock = [], payments = [], ordersBasePath = "/admin/orders", canProcess=true, canReviewPayments=true }: { order: Order; items: Item[]; stores?: Store[]; fulfilments?: Fulfilment[]; stock?: Stock[]; payments?: Payment[]; ordersBasePath?:string;canProcess?:boolean;canReviewPayments?:boolean }) {
  ...
  const editable = canProcess && orderDetailsAreEditable(saved);
  const selectableStatuses = allowedOrderStatuses(saved, order.fulfilmentMethod);
  const steps = order.fulfilmentMethod === "DELIVERY" ? deliverySteps : pickupSteps;
  ...
  const fulfilmentPayload = useMemo(() => items.filter((item) => assignments[item.id]).map((item) => ({ orderItemId: item.id, branchId: assignments[item.id], quantityReserved: item.quantity, quantityPacked: isStockFinalizedOrderStatus(status) ? item.quantity : 0, status: isStockFinalizedOrderStatus(status) ? "READY" : "RESERVED" })), [items, assignments, status]);
  ...
  return <main className="order-detail admin-order-detail">
    ...
    <section>
      ...
      <form className="order-status-form order-edit-form" onSubmit={submit}>
        <label>Order status<select value={status} disabled={!canProcess || selectableStatuses.length === 1} onChange={(event) => setStatus(event.target.value as OrderStatus)}>{selectableStatuses.map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></label>
        ...
        {canProcess?<div className={styles.actions}><button disabled={saving}>{saving ? "Saving…" : "Save order"}</button></div>:null}
      </form>
