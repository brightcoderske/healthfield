...
export function ProductCard({ product, wishlistActive=false, cartQuantity=0, returnTo, onAddToCart }: { product:ProductCardProduct; wishlistActive?:boolean; cartQuantity?:number; returnTo:string; onAddToCart?:(event:FormEvent<HTMLFormElement>, productId:number)=>void }) {
  ...
  return <article className="approved-product">
    <Link prefetch={false} className="approved-product-main" href={`/products/${product.id}`} aria-label={`View ${product.name}`}>
      <div className="approved-product-image">{discount>0&&<span className="discount-badge">Save {discount}%</span>}{showImage?<img src={product.imageUrl!} alt={product.name} loading="lazy" decoding="async" onError={()=>setImageFailed(true)}/>:<div className="product-image-missing"><Package/><small>Image pending</small></div>}</div>
      <div className="approved-product-info"><span className="product-card-name">{product.name}</span>{!!product.rating&&<div className="approved-rating" aria-label={`${product.rating.toFixed(1)} from ${product.reviewCount??0} reviews`}>★ {product.rating.toFixed(1)} <small>({product.reviewCount??0})</small></div>}</div>
    </Link>
