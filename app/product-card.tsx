...
export function ProductCard({ product, wishlistActive=false, cartQuantity=0, returnTo, onAddToCart }: { product:ProductCardProduct; wishlistActive?:boolean; cartQuantity?:number; returnTo:string; onAddToCart?:(event:FormEvent<HTMLFormElement>, productId:number)=>void }) {
  ...
  return <article className="approved-product">
    <a className="approved-product-main" href={`/products/${product.id}`} aria-label={`View ${product.name}`}>
      <div className="approved-product-image">{discount>0&&<span className="discount-badge">Save {discount}%</span>}{product.imageUrl?<img src={product.imageUrl} alt={product.name}/>:<div className="product-image-missing"><Package/><small>Image pending</small></div>}</div>
