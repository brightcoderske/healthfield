...
type SaleState = { orderNumber: string; total: number; state: "WAITING" | "VERIFYING" | "CONFIRMING" | "REVIEWING" | "CANCELLING" | "CANCELLED" | "FAILED" | "COMPLETE"; message: string; candidate?:TillCandidate|null; candidates?:TillCandidate[] };
...
export function WalkInSaleForm({ branches, products, stock, payment, backHref = "/staff" }: { branches: Branch[]; products: Product[]; stock: Stock[]; payment: PaymentOptions; backHref?: string }) {
  ...
  useEffect(() => {
    if (!saleState || !["WAITING", "CONFIRMING", "REVIEWING", "CANCELLING"].includes(saleState)) return;
    let cancelled = false;
    async function check() {
      ...
      if (data.paid || data.order?.paymentStatus === "PAID") {
        ...
        setSale((current) => current ? { ...current, state: "VERIFYING", candidate:data.candidatePayment, candidates:undefined, message: data.message || "Till payment found. Confirm the payer details with the customer." } : current);
      } else if (Array.isArray(data.candidatePayments) && data.candidatePayments.length) {
        setSale((current) => current ? { ...current, state:"VERIFYING", candidate:null, candidates:data.candidatePayments, message:data.message || "Choose the payment after confirming the payer name or receipt with the customer." } : current);
    ...
    function schedule() {
      if (cancelled) return;
      const delay = method === "MANUAL_MPESA" ? manualTillPollDelay(pollCount.current) : paymentPollDelay(pollCount.current);
  ...
  async function submitManualProof(event: FormEvent) {
    ...
    if (proof.length < 10) return setMessage("Enter the 10-character M-Pesa receipt code or paste the confirmation SMS.");
    setSending(true); setMessage("");
    const response = await fetch("/api/payments/manual", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ checkoutToken: checkoutToken.current, message: proof }) }).catch(() => null);
    const data = await response?.json().catch(() => ({}));
    if (!response?.ok) setMessage(data?.error || "The receipt could not be submitted for review.");
    else if (data.paid) setSale((current) => current ? { ...current, state: "COMPLETE", message: "Till payment confirmed and stock updated." } : current);
    else if (data.candidatePayment) setSale((current)=>current?{...current,state:"VERIFYING",candidate:data.candidatePayment,candidates:undefined,message:data.message||"Till payment found. Confirm the payer details with the customer."}:current);
    else if (Array.isArray(data.candidatePayments)&&data.candidatePayments.length) setSale((current)=>current?{...current,state:"VERIFYING",candidate:null,candidates:data.candidatePayments,message:data.message||"Choose the payment after confirming the payer name or receipt with the customer."}:current);
    else {
      pollCount.current = 0;
      setSale((current) => current ? { ...current, state: "REVIEWING", message: data.message || "Receipt submitted for administrator approval. Do not request another payment." } : current);
    }
    setSending(false);
  }
  ...
  async function confirmTillPayment(candidate:TillCandidate){
    setSending(true);setMessage("");
    const response=await fetch(`/api/payments/incoming/${candidate.id}/confirm-pos`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({checkoutToken:checkoutToken.current})}).catch(()=>null);
    const data=await response?.json().catch(()=>({}));
